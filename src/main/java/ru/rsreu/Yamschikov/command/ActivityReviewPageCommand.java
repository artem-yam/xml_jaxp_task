...
public class ActivityReviewPageCommand implements ActionCommand {
	@Override
	public GoToPageHandler execute(HttpServletRequest request) {
		...
		case EVENT:
			...
			if (event.isParticipation()) {
				...
				request.setAttribute("no", "disabled");
			} else if (!event.isParticipation()) {
				request.setAttribute("yes", "disabled");
		...
		String page = ("path.page.activity.review");
